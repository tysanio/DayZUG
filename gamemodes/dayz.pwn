...
#define ZOMBIE_PREFIX "DayZUG_Zombie_"
...
new randomMessages[][] =
{
	"*"COL_LIGHTBLUE" Buy Premium/V.I.P at the discord",
...
public OnPlayerWeaponEmpty(playerid, weaponid)
{
	...
	switch(GetPlayerWeapon(playerid))
	{
	    ...
	    case 279: wepname = "SPAZ-15 Shotgun", SendClientMessage(playerid, -1, ""chat" You ran out ammo! Your weapon has been dropped"), DayZSA_CreateLootsFunction("SPAZ-15 Shotgun",fPos[0],fPos[1],fPos[2]);
...
CMD:dropgun(playerid)
{
	if(GetPlayerWeapon(playerid) >= 1)
	{
		...
		switch(GetPlayerWeapon(playerid))
		{
		    ...
		    case 279: wepname = "SPAZ-15 Shotgun", SendClientMessage(playerid, -1, ""chat" You dropped your current weapon/tool!"), DayZSA_DropWeapon("SPAZ-15 Shotgun", GetPlayerAmmo(playerid),fPos[0],fPos[1],fPos[2]);
					...
					else
					{
						if(Loot[GetLootW_ID(playerid)][AntiDup] == 1)
						{
							switch(Loot[GetLootW_ID(playerid)][DropGunID])
							{
								...
								case 29323: AddSlotToInventoryWorld(playerid, "SPAZ-15 Shotgun", 1),GivePlayerWeaponEx(playerid, 279, Loot[GetLootW_ID(playerid)][DropGunAmmo]), DayZSA_DestroyLoot(GetLootW_ID(playerid)), Loot[GetLootW_ID(playerid)][AntiDup] = 0;
...
stock CreateTDS()
{
	...
	DayZSA_Web = TextDrawCreate(509.000000, 4.000000, "discord.gg/3wjFetUaN3 / v15.0");
...
function IsPlayerAimingTargetBodyPart(playerid, targetid, bodypart)
{
	...
	switch (GetPlayerWeapon(playerid))
	{
	    case 24, 29, 22, 23, 25, 26, 279, 28, 32: offset = 0.1122047500310059125919013005129;
		...
		if(!strcmp(ItemName,"Gin",true))
	 	{
	  	...
	  	if(!strcmp(ItemName,"SPAZ-15 Shotgun",true))
	 	{
			if(PlayerHasItem(playerid,"Shotgun Ammo"))
			{
				GivePlayerWeaponEx(playerid, 279, 20);
				RemoveSlotToInventory(playerid,"SPAZ-15 Shotgun",1);
		...
		if(!strcmp(ItemName,"Shotgun Ammo",true))
	  	{
			...
			if(GetPlayerWeapon(playerid) == 279)
			{
			    GivePlayerWeaponEx(playerid,279,20);
				SendClientMessage(playerid,-1,""chat" Added ammo to your 'SPAZ-15 Shotgun'");
				RemoveSlotToInventory(playerid,"Shotgun Ammo",1);
				if(!IsPlayerInAnyVehicle(playerid))
		    	{
					ApplyAnimation(playerid, "RIFLE", "RIFLE_load",4.1,0,1,0,1000,1);
...
{
    RemoveBuildingForPlayer(playerid, 1283, 0.0, 0.0, 0.0, 6000);
	RemoveBuildingForPlayer(playerid, 1315, 0.0, 0.0, 0.0, 6000);
	RemoveBuildingForPlayer(playerid, 1350, 0.0, 0.0, 0.0, 6000);
	RemoveBuildingForPlayer(playerid, 1284, 0.0, 0.0, 0.0, 6000);
	return 1;
}
...
function DayZSA_LootSetupMedium()
{
	...
	switch(random(35))
	{
		...
		case 11: format(str,sizeof(str),"SPAZ-15 Shotgun"),dayzmodel = 29323, rotx = 276.0, roty = 90.0, rotz = 90.0;
...
function DayZSA_CreateLootsDeadBody(playerid,Float:pXX,Float:pYX,Float:pZX)
{
	...
	if(PlayerHasItem(playerid, "SPAZ-15 Shotgun")) DayZSA_CreateLootsFunction("SPAZ-15 Shotgun",pXX+random(2),pYX+random(1),pZX);
...
function DayZSA_LootModelDropWeapon(objectname[])
{
	...
	if(strfind(objectname, "SPAZ-15 Shotgun", true) != -1) dayzsaid = 29323, rotx = 276.0, roty = 90.0, rotz = 90.0;
...
stock DayZSA_GetWeaponName(playerid)
{
	...
	switch(GetPlayerWeapon(playerid))
	{
	    ...
	    case 279: wepname = "SPAZ-15 Shotgun";
	    case 29: wepname = "MP5";
...
stock jBan(player_banned, player_banner, reason[], time = 0, system = 0)
{
	if(system == 0)
	{
		...
		if(gotBanned[player_banned] == 0)
		{
			...
			SendClientMessageToAll(-1, strkick);
            SendClientMessage(player_banned, -1, ""chat""COL_WHITE" If you feel this is a mistake please, visit the discord and open a ticket.");
...
function jban_OnQueryFinish(extraid)
{
	...
	if(rows)
	{
		...
		SendClientMessage(extraid, -1, ""COL_RED" *** POST YOUR APPEAL AT "COL_WHITE"discord.gg/3wjFetUaN3");
...
public CreateZombies()
{
	...
	if(SpawnedZombies < MAX_ZOMBIES)
	{
        ...
        if (random(2) == 1 && pednodes != 0)
		{
			...
			FCNPC_Spawn(thenpc_, 162, posNodes[0], posNodes[1], posNodes[2]);
		}
		else
		{
...
public FCNPC_OnRespawn(npcid)
{
	...
	if (random(2) == 1 && pednodes != 0)
	{
		...
		FCNPC_SetNodePoint(nodeid, point);
		FCNPC_GetNodePointPosition(nodeid, posNodes[0], posNodes[1], posNodes[2]);
		FCNPC_Spawn(npcid, 162, posNodes[0], posNodes[1], posNodes[2]);
	}
	else
	{
		point = random(vehnodes - 1);
